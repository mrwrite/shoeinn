...
import type { AppointmentEvent, AppointmentLocationUpdate, AppointmentSummary } from "../../types/booking";

const travelStatuses = new Set(["en_route_pickup", "picked_up", "out_for_delivery"]);
...
type Props = NativeStackScreenProps<CustomerStackParamList, "AppointmentDetail">;
...
const ProviderCard = ({ providerName, assignedAt }: { providerName: string; assignedAt: string }) => (
...
const buildStatusHistory = (events: AppointmentEvent[], currentStatus?: string) => {
...
const TrackingMap = ({ location }: { location: AppointmentLocationUpdate }) => {
  ...
  return (
    <MapView style={styles.map} provider={PROVIDER_GOOGLE} region={region}>
      <Marker
        ...
        title="Provider"
        description={location.recorded_at ? `Updated ${formatDateTime(location.recorded_at)}` : undefined}
      />
    </MapView>
  );
};
      ...
      <ScrollView contentContainerStyle={styles.container}>
        {!appointment ? (
          <View style={styles.center}>
            {appointmentQuery.isLoading ? <ActivityIndicator /> : <Text>Appointment not found</Text>}
          </View>
        ) : (
          <>
            <View style={styles.card}>
              <Text style={styles.title}>{appointment?.service_name ?? "Appointment"}</Text>
              <Text style={styles.meta}>{formatDateTime(appointment.start_time)}</Text>
              <View style={styles.chip}>
                <Text style={styles.chipText}>{statusLabels[appointment.status] ?? appointment.status}</Text>
              </View>
            </View>
              ...
              {appointment.address_line1 ? (
                <>
                  ...
                  <Text style={styles.value}>
                    {[appointment.address_line1, appointment.address_line2].filter(Boolean).join(", ")}
                  </Text>
                  <Text style={styles.meta}>
                    {[appointment.city, appointment.state, appointment.postal_code].filter(Boolean).join(" ")}
                  </Text>
                </>
              ) : null}
            </View>
            ...
            <View style={styles.card}>
              ...
              {statusHistory.map((item) => (
                <StatusRow key={item.status} label={statusLabels[item.status] ?? item.status} active={item.active} />
              ))}
            </View>
            ...
            {shouldPollLocation ? (
              <View style={styles.card}>
                ...
                {locationQuery.isLoading ? (
                  ...
                  <TrackingMap location={locationQuery.data} />
                ) : (
